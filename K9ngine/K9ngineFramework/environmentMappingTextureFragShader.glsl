...
layout (binding=1) uniform samplerCube cubemap_samp;

void main(){
	vec3 texColor = (texture(tex_samp, varyingTexCoord)).xyz;
	...
	vec3 r = -reflect(V, N);
	fragColor = (1.0 - cubeMapFactor)*vec4(ambient + diffuse + specular, 1.0) +
				cubeMapFactor * texture(cubemap_samp, r);
}