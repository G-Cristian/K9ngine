...
namespace K9 {
	namespace Graphics {
		void EnvironmentMappingTextureRenderer::draw(const World& world, const std::set<std::shared_ptr<const K9::Components::RenderingComponent>>& renderingComponents) const {
			...
			GLint cubeMapFactorLoc = getUniformLocation(_renderingProgram, "cubeMapFactor");
			programUniform1f(_renderingProgram, cubeMapFactorLoc, 1.0f);
			...
			for (auto it = renderingComponents.cbegin(); it != renderingComponents.cend(); ++it) {
				...
				if (renderingComponent != nullptr) {
					...
					if (model != nullptr) {
						...
						auto cubeMapTexture = _cubeMapTexture.lock();
						K9_ASSERT(cubeMapTexture != nullptr);
		...
		glm::mat4 EnvironmentMappingTextureRenderer::initModelMatrix(const K9::Components::RenderingComponent& renderingComponent) const {
		...
		glm::mat4 EnvironmentMappingTextureRenderer::initViewMatrix(const World& world) const {
			glm::mat4 v_mat(1.0f);
		...
		glm::mat4 EnvironmentMappingTextureRenderer::initProjMatrix(const World& world) const {
		...
		void EnvironmentMappingTextureRenderer::initLights(const World& world, const glm::mat4& viewMatrix) const {
		...
		void EnvironmentMappingTextureRenderer::initMaterial(GLint specularLoc, GLint shininessLoc, const glm::vec4& specular, float shininess)const {
