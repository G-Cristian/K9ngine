...
namespace K9 {
	namespace Graphics {
		void NormalMappingRenderer::draw(const World& world, const std::set<std::shared_ptr<const K9::Components::RenderingComponent>>& renderingComponents) const {
			//std::string error;
			...
			for (auto it = renderingComponents.cbegin(); it != renderingComponents.cend(); ++it) {
				...
				if (renderingComponent != nullptr) {
					...
					if (model != nullptr) {
						...
						activateArrayBuffer3f(2, model->getVertexTangents());
						activateArrayBuffer2f(3, model->getVertexTextureCoordinates());
						...
						initNormalMap(renderingComponent->getNormalMaterial());
		...
		glm::mat4 NormalMappingRenderer::initModelMatrix(const K9::Components::RenderingComponent& renderingComponent) const {
		...
		glm::mat4 NormalMappingRenderer::initViewMatrix(const World& world) const {
		...
		glm::mat4 NormalMappingRenderer::initProjMatrix(const World& world) const {
		...
		void NormalMappingRenderer::initLights(const World& world, const glm::mat4& viewMatrix) const {
		...
		void NormalMappingRenderer::initVectorColorMaterial(GLint loc, std::shared_ptr<const MaterialComponent> materialComponent)const {
		...
		void NormalMappingRenderer::initFloatColorMaterial(GLint loc, std::shared_ptr<const MaterialComponent> materialComponent)const {
			K9_ASSERT(materialComponent != nullptr);
			if (materialComponent != nullptr) {
		...
		void NormalMappingRenderer::initNormalMap(std::shared_ptr<const NormalMaterial> normalMaterial)const {
			K9_ASSERT(normalMaterial != nullptr);
			if (normalMaterial != nullptr) {
				auto texture = normalMaterial->getTexture();
				K9_ASSERT(texture != nullptr);
				if (texture != nullptr) {
					auto weakTexture = texture->getTexture();
					K9_ASSERT(!weakTexture.expired());
					if (!weakTexture.expired()) {
						auto txtr = weakTexture.lock();
						K9_ASSERT(txtr != nullptr);
						if (txtr != nullptr) {
							activateTexture02D(txtr->id());
						}
					}
				}
			}
